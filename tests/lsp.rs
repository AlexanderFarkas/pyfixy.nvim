...
impl Lsp {
    ...
    fn request(&mut self, method: &str, params: Value) -> Value {
        ...
        loop {
            ...
            if msg.get("method").and_then(Value::as_str) == Some("pyfixy/fixtureReturnTypes") {
                self.respond_to_fixture_return_types(msg);
            }
        }
    }

    fn respond_to_fixture_return_types(&mut self, msg: Value) {
        let Some(id) = msg.get("id").cloned() else {
            return;
        };
        let result: Vec<_> = msg
            .get("params")
            .and_then(|params| params.get("fixtures"))
            .and_then(Value::as_array)
            .into_iter()
            .flatten()
            .filter_map(|fixture| {
                Some(json!({
                    "uri": fixture.get("uri")?.clone(),
                    "name": fixture.get("name")?.clone(),
                    "type": "InferredClient",
                }))
            })
            .collect();
...
#[test]
fn ty_bridge_completion_caches_inferred_fixture_type_for_code_actions() {
    let tmp = TempDir::new().unwrap();
    let conftest = tmp.path().join("tests/conftest.py");
    let test = tmp.path().join("tests/test_app.py");
    write(
        &conftest,
        "import pytest\n@pytest.fixture\ndef admin_client(): pass\n",
    );
    write(&test, "def test_x(admin_client): pass\n");
    ...
    lsp.request(
        ...
        json!({
            "rootUri": url::Url::from_directory_path(tmp.path()).unwrap(),
            "initializationOptions": { "ty_bridge": true }
        }),
    );
    lsp.send(json!({
        "jsonrpc": "2.0",
        ...
        "params": {
            "textDocument": {
                ...
                "languageId": "python",
                "version": 1,
                "text": "def test_x(admin_client): pass\n"
            }
        }
    }));
    ...
    assert!(completion["result"]
        ...
        .unwrap()
        .iter()
        .any(|item| item["label"] == "admin_client"
            && item["insertText"] == "admin_client: InferredClient"));
    ...
    let quickfix = actions["result"]
        .as_array()
        .unwrap()
        .iter()
        .find(|action| action["title"] == "Add fixture type annotation")
        .expect("missing fixture type quickfix");
    let edits = quickfix["edit"]["changes"]
        .as_object()
        .unwrap()
        .values()
        .next()
    ...
    assert!(edits
        .iter()
        .any(|edit| edit["newText"] == ": InferredClient"));
}

#[test]
fn ty_bridge_code_action_can_query_inferred_fixture_type_without_prior_completion() {
    let tmp = TempDir::new().unwrap();
    let conftest = tmp.path().join("tests/conftest.py");
    let test = tmp.path().join("tests/test_app.py");
    write(
